...
def _load_papers_to_dict(by_year: bool = True) -> Tuple[Dict, List]:
    ...
    with open(DATA_FILE, newline='') as f:
        ...
        if by_year:
            return _h(reader)
        return _helper(reader)


def _helper(reader: csv.DictReader) -> Tuple[Dict, List]:
    d = ({}, [])
    for row in reader:
        # category = [x.strip() for x in row['Category'].split(':')]
        category = row['Category'].split(':')
        _remove_spaces(category)
        d_2 = d[0]
        for i in range(len(category)):
            if not category[i] in d_2:
                d_2[category[i]] = ({}, [])
            if i != len(category) - 1:
...
def _h(reader: csv.DictReader) -> Tuple[Dict, List]:
    d = ({}, [])
    for row in reader:
        year = row['Year']
        category = row['Category'].split(':')
...
def _remove_spaces(l: List) -> None:
